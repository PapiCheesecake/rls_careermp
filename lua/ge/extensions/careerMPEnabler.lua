...
local function onAfterVehicleRepaired(targetVehicleID)
	if not targetVehicleID then return end
	if not MPVehicleGE or not MPVehicleGE.isOwn(targetVehicleID) then return end

	local veh = be:getObjectByID(targetVehicleID)
	if not veh then return end

	-- Tiny delayed no-op paint resend to wake up BeamMP's part/edit sync
	-- without respawning or rebuilding the vehicle.
	core_jobsystem.scheduleJob(function()
		local invVehId = career_modules_inventory.getInventoryIdFromVehicleId(targetVehicleID)
		if not invVehId then return end

		local invVehData = career_modules_inventory.getVehicles()[invVehId]
		if not invVehData or not invVehData.config or not invVehData.config.paints then return end

		local paints = deepcopy(invVehData.config.paints)

		if paints[1] then
			extensions.core_vehicle_colors.setVehiclePaint(1, paints[1], targetVehicleID)
		end
		if paints[2] then
			extensions.core_vehicle_colors.setVehiclePaint(2, paints[2], targetVehicleID)
		end
		if paints[3] then
			extensions.core_vehicle_colors.setVehiclePaint(3, paints[3], targetVehicleID)
		end
	end, 0.25)
end
